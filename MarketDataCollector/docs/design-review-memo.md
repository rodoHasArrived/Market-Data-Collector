...
The Market Data Collector is an event-driven, layered system designed to capture and store high-fidelity market microstructure data (tick-by-tick trades, BBO quotes, and L2 depth) from multiple data providers including Interactive Brokers and Alpaca. The architecture emphasizes determinism, auditability, controlled change, and operational safety. It can run in both **live** (provider-connected) and **offline** (no provider dependency) modes with identical orchestration logic.
...
- Ensure clear separation between vendor integration (provider-specific code) and business logic (domain).
...
- Support multiple data providers through a unified interface.
...
- Exchange-certified market data reconstruction (feed limitations acknowledged).
...
- Provider credentials: stored in `appsettings.json`. Protect this file appropriately or migrate to environment variables / secret vault.
...
- Market data entitlements (especially depth) or API credentials
- Stable time sync (NTP)
- Log retention and disk monitoring
- Preflight checks (disk space, directory permissions, provider reachability)
...
|------|-------------|------------|
| Data gaps | Feed interruptions or missed updates | Integrity events + resubscribe procedures |
...
| Feed divergence | Multiple provider quotes may disagree | Preserve `StreamId`/`Venue`; pick a primary source |
