...
/// This algorithm uses tick-by-tick trade data and BBO quotes collected by MarketDataCollector
...
public class SampleLeanAlgorithm : QCAlgorithm
{
    ...
    /// <summary>
    /// Initialize the algorithm with custom MarketDataCollector data.
    /// </summary>
    public override void Initialize()
    {
        ...
        // Subscribe to custom MarketDataCollector trade data
        AddData<MarketDataCollectorTradeData>("SPY", Resolution.Tick);

        // Subscribe to custom MarketDataCollector BBO quote data
        AddData<MarketDataCollectorQuoteData>("SPY", Resolution.Tick);

        // Initialize indicators
        _sma = SMA(_symbol, 20, Resolution.Minute);

        Log("SampleLeanAlgorithm initialized with MarketDataCollector data feeds");
    }
    ...
    public override void OnData(Slice data)
    {
        ...
        if (data.ContainsKey("SPY") && data["SPY"] is MarketDataCollectorTradeData tradeData)
        {
            ProcessTradeData(tradeData);
        }

        // Process custom quote data
        if (data.ContainsKey("SPY") && data["SPY"] is MarketDataCollectorQuoteData quoteData)
        {
    ...
    /// <summary>
    /// Process trade data from MarketDataCollector.
    /// Demonstrates access to high-fidelity trade properties like aggressor side, conditions, etc.
    /// </summary>
    private void ProcessTradeData(MarketDataCollectorTradeData trade)
    {
        _tradeCount++;
    ...
    /// <summary>
    /// Process BBO quote data from MarketDataCollector.
    /// Demonstrates access to best bid/offer, spread, and exchange information.
    /// </summary>
    private void ProcessQuoteData(MarketDataCollectorQuoteData quote)
    {
