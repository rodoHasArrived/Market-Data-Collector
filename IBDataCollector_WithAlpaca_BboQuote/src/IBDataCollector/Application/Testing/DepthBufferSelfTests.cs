...
namespace IBDataCollector.Application.Testing;
...
public static class DepthBufferSelfTests
{
    ...
    {
        var buf = new MarketDepthCollector.SymbolOrderBookBuffer();
        var ts = DateTimeOffset.UtcNow;
        var sym = "TEST";

        // insert bid0, ask0
        var r1 = buf.Apply(new MarketDepthUpdate(ts, sym, 0, DepthOperation.Insert, OrderBookSide.Bid, 100, 10), out var snap1);
        if (r1 != DepthIntegrityKind.Unknown || snap1 is null) throw new Exception("Insert bid failed.");

        var r2 = buf.Apply(new MarketDepthUpdate(ts, sym, 0, DepthOperation.Insert, OrderBookSide.Ask, 101, 12), out var snap2);
        if (r2 != DepthIntegrityKind.Unknown || snap2 is null) throw new Exception("Insert ask failed.");
        ...
        if (r4 != DepthIntegrityKind.Unknown || snap4 is null) throw new Exception("Delete ask failed.");
    ...
    private static void Test_GapMarksStale()
    {
        ...
        if (r == DepthIntegrityKind.Unknown) throw new Exception("Expected integrity failure.");
