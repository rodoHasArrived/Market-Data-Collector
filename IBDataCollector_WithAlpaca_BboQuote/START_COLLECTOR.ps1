...
<#
START_COLLECTOR.ps1
IB Data Collector – Canonical Startup (Windows PowerShell)
...
$Src  = Join-Path $Root "src\IBDataCollector"
$Ui   = Join-Path $Root "src\IBDataCollector.Ui"
...
function Preflight {
  ...
  if (Test-Path $cfgPath) {
    try {
      ...
      if ($depth -gt 0) { Write-Host "[NOTE] L2 depth requires IB depth entitlements for venues." }
    } catch {
...
$collectorArgs = @(
  "run",
  "--project", (Join-Path $Src "IBDataCollector.csproj"),
...
if ($StartUi -eq "true") {
  ...
  $uiArgs = @(
    "run",
    "--project", (Join-Path $Ui "IBDataCollector.Ui.csproj"),
    "--configuration", $Config
  )
