...
public sealed class DiagnosticBundleService
{
        ...
        CancellationToken ct = default)
    {
        ...
        finally
        {
            ...
            {
                _log.Debug(ex, "Best effort cleanup of temp directory {TempDir} failed", tempDir);
            }
        }
    }
    ...
    private async Task CollectStorageInfoAsync(string tempDir, DiagnosticManifest manifest, CancellationToken ct)
    {
        ...
        if (Directory.Exists(_dataRoot))
        {
            ...
            {
                info.AppendLine("=== DISK SPACE ===");
                info.AppendLine("Disk info not available on this system");
            }
        }
    ...
    private static void ListDirectory(string path, StringBuilder sb, string indent, int maxDepth)
    {
        ...
        try
        {
            foreach (var dir in Directory.GetDirectories(path))
            {
                ...
                ListDirectory(dir, sb, indent + "  ", maxDepth - 1);
            }
