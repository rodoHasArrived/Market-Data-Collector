...
public sealed class PortableDataPackager
{
    ...
    private readonly PackageScriptGenerator _scriptGenerator = new();
        ...
        CancellationToken ct)
    {
        ...
        // Add README
        var readme = _scriptGenerator.GenerateReadme(manifest, options);
        ...
        {
            var dictionary = _scriptGenerator.GenerateDataDictionary(manifest);
        ...
        {
            var pythonScript = _scriptGenerator.GeneratePythonLoader(manifest);
            ...
            var rScript = _scriptGenerator.GenerateRLoader(manifest);
        ...
        if (options.GenerateImportScripts && options.ImportScriptTargets != null)
        {
            ...
            {
                var script = _scriptGenerator.GenerateImportScript(manifest, target);
    ...
    // Script generation methods extracted to PackageScriptGenerator class.
    // See PackageScriptGenerator.cs for: GenerateReadme, GenerateDataDictionary,
    // GeneratePythonLoader, GenerateRLoader, GenerateImportScript,
    // GeneratePostgreSqlImport, GenerateClickHouseImport, GenerateDuckDbImport,
    // GeneratePythonImport, GenerateRImport, GenerateSparkImport.
