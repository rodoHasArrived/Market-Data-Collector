...
public sealed class AnalysisExportService
{
        ...
        CancellationToken ct)
    {
        ...
        foreach (var group in GroupBySymbolIfRequired(sourceFiles, profile.SplitBySymbol))
        {
            ...
            var xlsxFiles = group.Select(f => new XlsxExportFormatter.XlsxSourceFile { Path = f.Path }).ToList();
            var recordCount = await XlsxExportFormatter.CreateXlsxFileAsync(
                outputPath, xlsxFiles, profile.MaxRecordsPerFile, ReadJsonlRecordsAsync, _log, ct);
