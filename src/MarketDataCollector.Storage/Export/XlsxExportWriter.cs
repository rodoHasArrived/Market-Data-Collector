...
/// <summary>
/// Writes market data records to XLSX format using Open XML structure.
/// Extracted from AnalysisExportService to isolate XLSX-specific complexity.
/// </summary>
internal sealed class XlsxExportWriter
{
    private readonly ILogger _log = LoggingSetup.ForContext<XlsxExportWriter>();

    /// <summary>
    /// Creates an XLSX file from the provided source records.
    /// </summary>
    public async Task<long> WriteAsync(
        string outputPath,
        IAsyncEnumerable<Dictionary<string, object?>> records,
        int? maxRecords,
        CancellationToken ct)
    {
        ...
        await foreach (var record in records.WithCancellation(ct))
        {
            ...
            // Respect Excel row limit
            if (maxRecords.HasValue && recordCount >= maxRecords.Value)
            {
