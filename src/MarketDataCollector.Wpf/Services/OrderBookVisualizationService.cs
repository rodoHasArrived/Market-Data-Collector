...
public sealed class OrderBookVisualizationService : IDisposable
{
    ...
    private readonly ConcurrentDictionary<string, List<OrderBookVizSnapshot>> _snapshotHistory = new();
    ...
    public async Task SubscribeAsync(string symbol, int depthLevels = 10, CancellationToken ct = default)
    {
        if (!_orderBooks.ContainsKey(symbol))
        {
            ...
            _snapshotHistory[symbol] = new List<OrderBookVizSnapshot>();
        }
    ...
    public void RecordTrade(string symbol, decimal price, decimal size, TradeSide side)
    {
        ...
        var trade = new OrderBookTradeRecord
        {
    ...
    private void AggregateSnapshots(object? state)
    {
        foreach (var (symbol, orderBook) in _orderBooks)
        {
            ...
            var snapshot = new OrderBookVizSnapshot
            {
                Timestamp = DateTime.UtcNow,
    ...
    private void OnTradeReceived(string symbol, OrderBookTradeRecord trade)
    {
...
public class OrderBookState
{
    ...
    public List<OrderBookTradeRecord> RecentTrades { get; } = new();
...
/// </summary>
public class OrderBookVizSnapshot
{
...
{
    public string Symbol { get; set; } = string.Empty;
    public List<OrderBookTradeRecord> RecentTrades { get; set; } = new();
    public decimal TotalVolume { get; set; }
    public decimal BuyVolume { get; set; }
    public decimal SellVolume { get; set; }
...
/// </summary>
public class OrderBookTradeRecord
{
    public DateTime Timestamp { get; set; }
    public decimal Price { get; set; }
    public decimal Size { get; set; }
...
{
    public string Symbol { get; set; } = string.Empty;
    public OrderBookTradeRecord Trade { get; set; } = new();
}
