...
public sealed class DiagnosticsService
{
    ...
    public async Task<PreflightResult> RunPreflightCheckAsync(CancellationToken ct = default)
    {
        ...
        var providerResponse = await _apiClient.GetWithResponseAsync<DiagProviderStatusResponse>(
    ...
    /// </summary>
    public async Task<DiagSystemMetrics> GetSystemMetricsAsync(CancellationToken ct = default)
    {
        var response = await _apiClient.GetWithResponseAsync<DiagSystemMetrics>(
            UiApiRoutes.DiagnosticsMetrics,
            ct);

        return response.Data ?? new DiagSystemMetrics();
    }

    /// <summary>
    /// Validates a specific configuration setting.
    /// </summary>
    public async Task<DiagValidationResult> ValidateConfigurationAsync(
        string settingName,
        string value,
        CancellationToken ct = default)
    {
        var response = await _apiClient.PostWithResponseAsync<DiagValidationResult>(
            UiApiRoutes.DiagnosticsValidate,
            new { setting = settingName, value },
            ct);

        return response.Data ?? new DiagValidationResult { Valid = false, Error = response.ErrorMessage };
    }

    /// <summary>
    /// Tests connectivity to a specific provider.
    /// </summary>
    public async Task<DiagProviderTestResult> TestProviderAsync(string providerName, CancellationToken ct = default)
    {
        var response = await _apiClient.PostWithResponseAsync<DiagProviderTestResult>(
            UiApiRoutes.WithParam(UiApiRoutes.DiagnosticsProviderTest, "providerName", providerName),
            null,
            ct);

        return response.Data ?? new DiagProviderTestResult
        {
            Success = false,
    ...
    /// </summary>
    public async Task<DiagConfigValidationResult> ValidateFullConfigAsync(CancellationToken ct = default)
    {
        ...
        if (response.Success && response.Data != null)
        {
            return new DiagConfigValidationResult
            {
                Success = true,
...
public class DiagSystemMetrics
{
...
public class DiagValidationResult
{
    public bool Valid { get; set; }
    public string? Error { get; set; }
...
public class DiagProviderTestResult
{
    public bool Success { get; set; }
    public string? Error { get; set; }
...
public class DiagProviderStatusResponse
{
    public List<DiagProviderInfo>? Providers { get; set; }
}

public class DiagProviderInfo
{
    public string Name { get; set; } = string.Empty;
...
// Config Validation
public class DiagConfigValidationResult
{
    public bool Success { get; set; }
    public string? Error { get; set; }
    public bool IsValid { get; set; }
