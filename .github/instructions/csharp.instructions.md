...
6. Use `System.Threading.Channels.BoundedChannel` for producer-consumer queues; always specify `BoundedChannelFullMode.DropOldest`.
7. Prefer `Span<T>` and `Memory<T>` for buffer operations in hot paths; avoid LINQ `.ToList()` / `.Select()` allocations per-tick.
8. All domain exceptions must derive from `MarketDataException`; never throw bare `Exception` or `ApplicationException`.
