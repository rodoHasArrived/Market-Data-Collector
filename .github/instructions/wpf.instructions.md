---
applyTo: "src/MarketDataCollector.Wpf/**"
---
...
2. All state, commands, and orchestration belong in a ViewModel that inherits from `BindableBase` (located at `src/MarketDataCollector.Wpf/ViewModels/BindableBase.cs`).
3. Expose data to the View via bindable properties using `SetProperty<T>` — never set UI element properties directly from code-behind (e.g., `MyLabel.Text = ...`).
...
8. Never introduce dependencies on `MarketDataCollector.Wpf` types inside `Ui.Services` or `Ui.Shared` — that is a reverse dependency violation.
